...
public abstract class Player extends ColEntity {
	...
	//Update that gets fired even during pause. Used to transition.
	...
	public void camUpdate() {
		...
		if (camera) {
			...
			if (gs.cam.viewportWidth < gs.w) while (!(camx - gs.cam.viewportWidth / 2 >= 0 && camx + gs.cam.viewportWidth / 2 <= gs.w)) {
			...
			if (gs.cam.viewportHeight < gs.h) while (!(camy - gs.cam.viewportHeight / 2 >= 0 && camy + gs.cam.viewportHeight / 2 <= gs.h)) {
	...
	protected void updatePosition() {
		...
		for (int i = 0; i < gs.tilemap.getLayers().get("Collisions").getObjects().getCount() && UP; i++) {
			if (gs.tilemap.getLayers().get("Collisions").getObjects().get(i).getName() != null) if (gs.tilemap.getLayers().get("Collisions").getObjects().get(i).getName().equals("block")) continue;
		...
		if (this instanceof IceCharacter && inWater && dy < 0) {
			dy = 0;
			canSwing = true;
			didSwing = false;
			if (UP) {
		...
		//Vines
		if (!ropeSwing && trueUP && canSwing) for (ColEntity c: ((PlayGameState) gs).entities) {
		...
		for (ColEntity en: gs.entities) {
			if (en instanceof Block) {
				...
				//TODO fix weird -0.4 dy when standing on a block
				...
				if (dy < 0) {
					if (en.colRect.contains(mapTangle.x, mapTangle.y + dy) || en.colRect.contains(mapTangle.x + mapTangle.width, mapTangle.y + dy)) {
						dy = 0;
						if (UP) {
							dy += jumpspeed;
							jumping = true;
	...
	public void draw(SpriteBatch g) {
		//TODO move platforms from player to the gamestate
	...
	public void keyPressed(int k) {
		if (transition == null) {
			if (k == Keys.A && transition == null && oldanim == null) {
				lastKey = 0;
				LEFT = true;
				if (!ropeSwing) facingRight = false;
			}
			if (k == Keys.D && oldanim == null) {
				lastKey = 1;
				RIGHT = true;
				if (!ropeSwing) facingRight = true;
			}
			if (k == Keys.SPACE && (dy == 0 || slopeadj)) {
				UP = true;
